...
def main():
    ...
    st.set_page_config(
        ...
        page_icon="Drowsiness Detection",
    ...
    # App title
    st.markdown("<div class='title'>Driver Monitoring System with Yolov8</div>", unsafe_allow_html=True)
    ...
    <div style='text-align: center;'>
        <p>Welcome to our Drowsiness Detection App! Our app uses the <a href='https://github.com/ultralytics/ultralytics'>YOLOv8</a> detection model, which is trained on the <a href='https://universe.roboflow.com/augmented-startups/drowsiness-detection-cntmz/dataset/2'>Drowsiness Detection Image Dataset</a> - a dataset of images that can detect Drowsiness and Awakeness.</p>
    ...
    with col1:
        model_type = st.radio("Select Model Type", ("Drowsiness Detection", "General"), index=0)

    models_dir = "general-models" if model_type == "General" else "drowsiness-models"
    ...
    with col1:
        with st.expander("What is General?"):
            ...
            st.caption("Please note that this model may not be optimized specifically for drowsiness detection.")
            st.caption("For accurate fire and smoke detection, it is recommended to choose the Drowsiness Detection model.")
    ...
    with col2:
        with st.expander("Size Information"):
            st.caption("Models are available in different sizes, indicated by s, m, and l.")
            st.caption("- s: Small")
            st.caption("- m: Medium")
            st.caption("- l: Large")
